...
Page({
  ...
  shengChengHangXian:function(){
    if (shengChengHangXianFlag){
      this.setData({
      mapViewDisplay: 1,
  ...
  generateNavLine: function () {
    ...
    var basePoint = this.basePoint(jingduMinPoint.latitude, jingduMinPoint.longitude, 0, weiduMinPoint.latitude, this.data.polyline[0].points);

    //第一次根据基准点求偏移点，偏移量是幅宽的一半
    var basePointOffset = this.ComputeOffset(basePoint.latitude, basePoint.longitude, 50, 0 + 90);
    ...
    for (var i = 0; i < length; i++) {
      //从偏移点出发的射线和作业区的交点
      tempCrossPointArray = LineCross(basePointOffset.latitude, basePointOffset.longitude, 0, this.data.operationArray[i].latitude, this.data.operationArray[i].longitude, this.data.operationArray[i + 1].latitude, this.data.operationArray[i + 1].longitude)
    ...
    while (crossPoints.length > 0) {
      ...
      //根据偏移点求偏移点，偏移量是幅宽
      basePointOffset = this.ComputeOffset(basePointOffset.latitude, basePointOffset.longitude,100, 0 + 90);
